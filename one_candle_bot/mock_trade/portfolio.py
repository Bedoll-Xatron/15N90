...
class Portfolio:
    ...
    def sell(self, ticker: str, current_price: float, reason: str = "Exit"):
        if ticker not in self.positions:
            return None
            
        pos = self.positions[ticker]
        net_pnl = pos.pnl(current_price)
        ...
        self.save()
        self._record_history(pos, current_price, net_pnl, reason)
        logger.info(f"가상 매도: {pos.name}({ticker}) @ {current_price:,.0f} | 손익: {net_pnl:,.0f} ({reason})")
        return net_pnl

    def _record_history(self, pos: Position, exit_price: float, pnl: float, reason: str):
